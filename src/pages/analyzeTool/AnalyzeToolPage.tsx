...
const AnalyzeToolPage = () => {
  ...
  const [analyseProgress, setAnalyseProgress] = useState<ProgressStatus>({sortedIndex: 0, total: 0, path: ''});
  ...
  useEffect(() => {
    // Listeners
    window.electron.analyzeProgress.addAnalyzeProgressListener((_event, progress) => {
  ...
  return (
    <main>
      <section className="analyze-tool-section container">
        ...
        {analyzeError && <div className="progress-error container">
          <h3 className='progress-error__name'>An error has occurred. Please try again.</h3>
          <p className="progress-error__message">{analyzeError.name} : {analyzeError.message}</p>
        </div>}

        {analyzeResults?.length !== 0 && isAnalyzing && 
        <div className='progress-error container'>
          <h3 className="progress-error__name">Scanning results: ({analyzeResults.length})</h3>
          ...
          <ul className="analyze-results__list">
          {
            analyzeResults.map((file, index) => {
              return (
                <li className='analyze-results__item' key={index} onClick={() => handleOpenInFileExplorer(file.path)}>
                  {
                    <>
          ...
          {analyzeResults.length === 0 && isAnalyzing && <div className='progress-error container'>
          <h3 className="progress-error__name" style={{color: 'limegreen'}}>Scanning results: ({analyzeResults.length})</h3>
          <p className="progress-error__message">
          Everything is in order ✔️
          </p>
          </div>}
